...
describe('OrderableDisplayCategoryResource', function() {

    beforeEach(function() {
        this.OpenlmisResourceMock = jasmine.createSpy('OpenlmisResource');

        var OpenlmisResourceMock = this.OpenlmisResourceMock;
        module('referencedata-orderable-display-category', function($provide) {
    ...
    it('should extend OpenlmisResource', function() {
        new this.OrderableDisplayCategoryResource();

        expect(this.OpenlmisResourceMock).toHaveBeenCalledWith('/api/orderableDisplayCategories', {
            paginated: false
        });
    });
});